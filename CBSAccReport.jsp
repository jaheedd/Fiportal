...
<script type="text/javascript">
	...
	function generateBranchReport(format) {
		var action = "";
		if (format == 'Excel')
			action = "XslReportOfCbsAcBranch.action";
		else if (format == 'PDF')
			action = "PdfReportOfCbsAcBranch.action";
		...
		document.forms['cbsAccForm'].action = action + parameters;
		document.forms['cbsAccForm'].submit();
	}
	...
	function generateBCReport(format) {
		var action = "";
		if (format == 'Excel')
			action = "XslReportOfCbsAcBc.action";
		else if (format == 'PDF')
			action = "PdfReportOfCbsAcBc.action";
	...
	$
			.subscribe(
					'onGridCompleteBranch',
					function(event, data) {
						var records = $("#branchGrid").jqGrid("getGridParam",
								"records");
						var userdata = $("#branchGrid").jqGrid("getGridParam",
								"userData");
						var uploaded = userdata.bcCode;
						var notUploaded = parseInt(records - uploaded);
						$("#cbsAccBranchTblSummary")
								.html(
										"<tr><td>TOTAL NUMBER OF A/C GENERATED</td><td align='right'> "
												+ toInteger(records)
												+ "</td></tr>"
												+ "<tr><td>TOTAL NUMBER OF FP UPLOADED</td><td align='right'> "
												+ toInteger(uploaded)
												+ "</td></tr>"
												+ "<tr><td>TOTAL NUMBER OF FP NOT UPLOADED</td><td align='right'> "
												+ toInteger(notUploaded)
												+ "</td></tr>");
					});
	$
			.subscribe(
					...
					function(event, data) {
						var records = $("#bcGrid").jqGrid("getGridParam",
								"records");
						var userdata = $("#bcGrid").jqGrid("getGridParam",
								"userData");
						var uploaded = userdata.bcCode;
						var notUploaded = parseInt(records - uploaded);
						$("#cbsAccBCTblSummary")
...
<body class="tabsBody">
	...
	<s:form name="cbsAccForm" id="cbsAccForm" method="post"
		theme="css_xhtml">
		<sj:tabbedpanel id="taskTabs">
			<sj:tab target="divBranch" label="Branch">
				...
				<div id="divBranch">
					...
					<table cellpadding="6" class="filterTable-tabs" width="70%">
						<tr>
							...
							<sj:select label="Select Branch" indicator="branchListIndicator"
									headerKey="-1" id="branchSelect" headerValue="All" reloadTopics="reloadBranchList"
									labelposition="left" list="branchList" listValue="displayValue" labelSeparator="<br>"
									listKey="key" href="%{loadBranchList}" name="branchCode"
									onchange="onBranchChange()" /></td>
							<td colspan="2">
									<sj:datepicker name="startDateBranch" id="dateStartBranch"
						...
						<tr>
							...
							<td><img id="acListIndicator" class="indicator-small"
								src="images/indicator_small.gif" /> <s:url var="acListUrl"
									...
									id="acSelectBranch" formIds="cbsAccForm"
					...
					<s:url var="branchSelection" action="branchSelection.action" />
					<sjg:grid id="branchGrid" caption="Branch Wise CBS Accounts"
						dataType="json" href="%{branchSelection}" pager="true"
						autowidth="true" shrinkToFit="false" gridModel="gridModel"
						rowList="10,15,20,25" rowNum="15" sortname="bcCode"
						sortable="true" viewrecords="true" rownumbers="true"
						formIds="cbsAccForm" reloadTopics="reloadBranchGrid"
			...
			icon: 'ui-icon-document',caption : 'Export to PDF',topic:'ExportToPDFBranch'}}">
						...
						<sjg:gridColumn name="bcCode" index="bcCode" title="BC Code"
							sortable="true" width="60" search="true"
							searchoptions=" {sopt:['eq','cn','bw']}" />
						...
						<sjg:gridColumn name="eamDate" index="eamDate"
							title="Authorization Date" sortable="true" formatter="date"
						...
						<sjg:gridColumn name="documentNo" index="documentNo"
							title="ID Number" sortable="true" width="80" search="true"
							searchoptions=" {sopt:['cn','bw','eq']}" />
						<sjg:gridColumn name="cbsAccNo" index="cbsAccNo"
							title="CBS A/c No." sortable="true" width="80" search="true"
							searchoptions=" {sopt:['cn','bw','eq']}" />
						<sjg:gridColumn name="eamRespFlag" index="eamRespFlag"
							title="Uploaded  FP " sortable="true" formatter="checkbox"
							width="50" search="true" searchtype="select"
							searchoptions=" {sopt:['eq'],value:'Y:Uploaded FP; :Not Uploaded FP'}" />
						<sjg:gridColumn name="fingerPrintId" index="fingerPrintId"
							title="Captured Fingerprint IDs" sortable="true" width="140"
					...
					<table width="100%">
						<tr>
							<td width="60%"><table class="tblTotalSummary"
									id="cbsAccBranchTblSummary"></table></td>
						</tr>
			...
			<sj:tab target="divBC" label="BC ">
				<div id="divBC">
					...
					<table cellpadding="6" class="filterTable-tabs" width="70%">
						<tr>
							...
							<td width="120px"><img id="bcListIndicator" class="indicator-small"
								src="images/indicator_small.gif" />
							<sj:select label="Select BC" reloadTopics="reloadBCList" indicator="bcListIndicator"
									headerKey="-1" labelposition="left" headerValue="All" labelSeparator="<br>"
									id="bcSelect" list="bcList" href="%{loadBCList}"
									listValue="displayValue" listKey="key" name="bcCode"
									formIds="custWithAdharForm" onchange="onBCChange()" /></td>
							<td colspan="2">
									<sj:datepicker name="startDateBC" id="dateStartBC"
										labelposition="left" showOn="focus" label="From Date"
										labelSeparator="<br>" onChangeTopics="validateBCDate"
										displayFormat="dd/mm/yy" readonly="false" maxDate="today"
										onkeydown="return datepickerKeyDown(event);" /></td>
								<td>	<sj:datepicker name="endDateBC" id="dateEndBC" disabled="false"
						...
						<tr>
							...
							<td><img id="pmListIndicatorBC" class="indicator-small"
								src="images/indicator_small.gif" />
							<s:url var="pmListUrl" action="loadPMList" /> <sj:select indicator="pmListIndicatorBC"
							...
							<td><img id="acListIndicatorBC" class="indicator-small"
								src="images/indicator_small.gif" /> <s:url var="acListUrl"
									...
									id="acSelectBC" formIds="cbsAccForm"
					...
					<s:url var="cbsAccByBC" action="cbsAccByBC.action"
						includeParams="all" />
					<sjg:grid id="bcGrid" caption="BC Wise CBS Accounts"
						dataType="json" pager="true" sortname="bcCode" viewrecords="true"
						autowidth="true" shrinkToFit="false" href="%{cbsAccByBC}"
						gridModel="gridModelBC" rowList="10,15,20,25" rowNum="15"
						rownumbers="true" formIds="cbsAccForm" sortable="true"
			...
			icon: 'ui-icon-document',caption : 'Export to PDF',topic:'ExportToPDFBC'}}">
						...
						<sjg:gridColumn name="docType" index="docType"
							...
							searchoptions=" {sopt:['cn','bw','eq']}" />
						<sjg:gridColumn name="documentNo" index="documentNo"
							title="ID Number" sortable="true" width="110" search="true"
							searchoptions=" {sopt:['cn','bw','eq']}" />
						<sjg:gridColumn name="eamDate" index="eamDate" width="60"
							title="Authorization Date" sortable="true" formatter="date"
							formatoptions="{newformat : 'd-M-y'}" search="true"
							searchoptions="{sopt:['eq','lt','gt'],dataInit:dateField}" />
						<sjg:gridColumn name="cbsAccNo" index="cbsAccNo"
							title="CBS A/c No." sortable="true" width="110" search="true"
							searchoptions=" {sopt:['cn','bw','eq']}" />
						<sjg:gridColumn name="eamRespFlag" index="eamRespFlag" width="60"
							title=" Uploaded  FP " sortable="true" formatter="checkbox"
					...
					<table width="100%">
						<tr>
							<td width="60%"><table class="tblTotalSummary"
									id="cbsAccBCTblSummary"></table></td>
						</tr>
