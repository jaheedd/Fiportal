...
<script type="text/javascript">
	function generateReport(format) {
		var action = "";
		if (format == 'Excel')
			action = "XslReportOfBrCustGrid.action";
		else if (format == 'PDF')
			action = "pdfReportOfBrCustGrid.action";
		var postData = $("#custGridID").jqGrid("getGridParam", "postData");
		...
		var parameters = "?sidx=" + postData.sidx + "&sord=" + postData.sord
				...
				+ filterString;
		document.forms['brWiseCustForm'].action = action + parameters;
		document.forms['brWiseCustForm'].submit();
	}
	...
	$.subscribe('loadGrid', function(event, data) {
		$("#custGridID").jqGrid("setGridParam", {
			records : 0,
			page : 1
		});
		$.publish("reloadGrids");
	...
	$.subscribe('onGridComplete', function(event, data) {
		var records = $("#custGridID").jqGrid("getGridParam", "records");
		$("#brWiseCustomerTblSummary").html(
...
<body>
	<hr color="#F28500">
	<s:form id="brWiseCustForm" method="post" theme="css_xhtml">
		...
		<s:url var="brCustGridURL" action="brCustGrid.action" />
		<sjg:grid id="custGridID" caption="CBS Customer Report"
			dataType="json" sortable="true" autowidth="true"
			formIds="brWiseCustForm" href="%{brCustGridURL}" pager="true"
			...
			<sjg:gridColumn name="cbsAcc" index="cbsAcc" title="CBS Account No."
				...
				searchoptions=" {sopt:['cn','bw','eq']}" />
			<sjg:gridColumn name="accountOpenDate" index="accountOpenDate"
				title="A/c Open Date" formatter="date"
		...
		<table width="100%">
			<tr>
				<td width="60%"><table class="tblTotalSummary"
						id="brWiseCustomerTblSummary"></table></td>
			</tr>
		</table>
